@php
    use Modules\Survey\Entities\SurveyAssign;if (!empty($course->survey->questions)){
        $totalQus =$course->survey->questions->count();
    }else{
        $totalQus =0;
    }
     $assign_info= SurveyAssign::where('survey_id',$course->survey->id)->where('user_id',auth()->user()->id) ->first();
@endphp
